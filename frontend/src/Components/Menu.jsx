...
function MenuSection() {
  ...
  return (
    <div className="bg-gray-100">
      ...
      <motion.h2 className="text-3xl font-bold text-gray-800 mb-10 text-center"
      ...
      <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="flex flex-wrap justify-center gap-6 w-full px-2 mt-8">
            {searchedFood.filter(item => item.category === category ).map((product , index)=>{
                return <motion.div key={product.id} className="bg-white shadow-md rounded-xl p-4 flex flex-col items-center text-center w-[270px] sm:w-60"
            ...
            viewport={{ once: true, amount: 0.2 }}
                >
                    <img src={product.img} alt={product.name} className="w-40 h-40 object-cover rounded-full border-4 border-red-100 shadow-sm"/>
                    <h3 className="mt-4 text-lg font-semibold text-gray-800">{product.name}</h3>
                    <p className="text-sm text-gray-600 mt-1">{product.description}</p>
                    <p className="text-red-500 font-bold text-base mt-2">${product.price}</p>
                    <button className="mt-2.5 px-6 py-3 bg-red-500 text-white font-medium rounded-full hover:bg-red-600 transition">Add to Cart</button>
                </motion.div>
            })}
        </div>
    </div>
