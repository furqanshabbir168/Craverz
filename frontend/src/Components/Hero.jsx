...
function Hero() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setFirstLoad(false); // after first change, disable animations
    }, 8000);
  ...
  return (
    <div
      className="relative h-[500px] sm:h-[650px] w-full bg-cover bg-center"
      ...
      <div className="relative z-10 flex flex-col-reverse lg:flex-row items-center justify-between px-4 sm:px-8 lg:px-24 py-10 h-full gap-8 sm:gap-14">
        ...
        <div className="text-center lg:text-left flex flex-col items-center lg:items-start gap-3 max-w-[650px]">
          {firstLoad ? (
            ...
            <>
              <h2 className="text-orange-400 text-xl sm:text-2xl font-medium">
                Welcome to Cravez
              </h2>
        ...
        {firstLoad ? (
          ...
          <div className="w-[220px] sm:w-[300px] lg:w-[400px] h-[220px] sm:h-[300px] lg:h-[400px] flex items-center justify-center rounded-full overflow-hidden">
            <img
              src={current.image}
              alt="Hero Food"
              className="w-full h-full object-contain"
