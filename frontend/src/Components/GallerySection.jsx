...
function GallerySection() {
  ...
  return (
    <div className="flex flex-col items-center p-4 mx-auto">
      ...
      {/* Gallery Items */}
      <div className="grid grid-cols-2 gap-4 sm:flex sm:flex-wrap sm:justify-center sm:gap-6 w-full px-2">
        {Gallery.filter((item) => item.atHomePage === true).map(
          (item, index) => (
            <motion.div
              key={item.id}
              ...
              viewport={{ once: true, amount: 0.2 }}
              className="relative group bg-white shadow-md flex flex-col items-center text-center overflow-hidden rounded-md w-full sm:w-auto"
            >
              <img
                src={item.img}
                alt="Gallery Photo"
                className="w-full sm:w-[300px] h-[180px] sm:h-[300px] object-cover border-red-100 shadow-sm transition-transform duration-500 group-hover:scale-110"
              />
